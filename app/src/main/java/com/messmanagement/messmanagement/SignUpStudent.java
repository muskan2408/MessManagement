...
public class SignUpStudent extends AppCompatActivity {
    TextInputEditText name,email,mobileno,pass,confirmpass,collegename;
    Button signup;
...
//                progressDialog.show();
                progressDialog=new ProgressDialog(SignUpStudent.this);
                progressDialog.setTitle("Creating Account");
                progressDialog.setMessage("Please wait while we create your account");
                progressDialog.setCancelable(false);
                progressDialog.show();
...
//                startActivity(i);
    ...
    private void startRegister(String collegename, String name, String email, String mobileno, String pass, String confirmpass) {
        ...
        if(name.isEmpty()){
            progressDialog.dismiss();
            this.name.setError("Email is required");
       ...
       if(mobileno.isEmpty()){
        ...
        Call<Model> call=retrofitInterface.registerstudent(collegename,name,mobileno,email,pass);
...
//                }
                if(model.getMsg().equals("user created")){
                    //  progressDialog.cancel();
                    new AlertDialog.Builder(SignUpStudent.this).setTitle("User Created Successfully")
                            .setMessage("Please Login to Continue")
                            .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    Intent intent=new Intent(SignUpStudent.this,SignInStudent.class);
                                    startActivity(intent);
                                    finish();
                                }
                            }).show();
                ...
                {
                    // progressDialog.dismiss();
                    Toast.makeText(SignUpStudent.this, "Some Error occured, Please try Again After Some Time! ", Toast.LENGTH_SHORT).show();
                }}

            @Override
            public void onFailure(Call<Model> call, Throwable t) {
                progressDialog.dismiss();
                Toast.makeText(SignUpStudent.this, "Error !!!!!!", Toast.LENGTH_SHORT).show();
            }
        });
